...
export class Envelope3D extends Sampler3D {
    ...
    setAxis(
        ...
        return super.configure(this.getAlgo(), axeName, property, {
            n,
            min,
            max,
            reverse,
    ...
    set property(name: string) {
        if (validPropertyNames.includes(name) === false) {
            // see below for validNames...
            throw new Error(
                `Invalid property named ${name}. Should be in ${validPropertyNames}`,
            )
        }
        this._property = name
    }
    ...
    getPositions(normalize = true) {
        ...
        if (normalize) {
            ...
            const normalize_ = (v: number, i: number) =>
                (v - m[i].min) / (m[i].max - m[i].min) // fct

            this.forEach((p) => p.forEach((v, i) => m[i].add(v)))
            this.forEach((p) =>
                p.forEach((v, i) => (values[l++] = normalize_(v, i))),
            )
        } else {
